...
req(c("sfsmisc","deSolve","data.table","boot","rmarkdown","knitr","stringr","reshape"))
...
calculate.rates <- function(t, x, vparameters, reported,beta.replacement=-99,time.to.report=5){
  npop=x[1]
  S = x[2]  # the value of S at time t
  E = x[3]
  I = x[4]  # the value of I at time t
  R = x[5]  # the value of R at time t
  D = x[6]
  T = x[7]
  TR = x[8]
  TD = x[9]
  ...
  out = c(gamma.leaving.I,beta,dnpop,dS,dE,dI,dR,dD,dT,dTR,dTD)
  return(out)
}
...
SEIR <- function(simulation.start,simulation.end,R0,time.incubation,time.infectedL,time.infectedD,cfr,
                 npop,E_0=1,I_0=1,R_0=0,D_0=0,T_0=0,TR_0=0,TD_0=0,mod,reported.to.date,referenceday=1,reduced.day=1,reduced.scenario=FALSE,time.to.report=5){
#  npop = 5000000
  ...
  inits = c(npop=npop,S=S_0,E=E_0,I=I_0,R=R_0,D=D_0,T=T_0,TR=TR_0,TD=TD_0)
...
reduce.options <- function(final.opts){
  ...
  if(sum(collapsed.final.opts$p==0)>num.keep){
    boundary <- collapsed.final.opts$p[order(-collapsed.final.opts$p)][num.keep]
    if(sum(collapsed.final.opts$p>boundary)>0){
	    collapsed.final.opts$p[collapsed.final.opts$p<boundary] <- 0
    }
  }
...
SEIR.second.step.against.real.data.multiple <- function(simulation.start,simulation.end,last.step,R0,data,mod){ 
  ...
  for(i in 1:length(last.step)){
    ...
    names(opts) <- c("R0","npop","E_0","I_0","R_0","D_0","T_0","TR_0","TD_0")
...
modelSEIR <- function(data,npop,opts.R0,opts.E_0,opts.I_0){
  ...
  data$day <- as.numeric(data$Date-min(data$Date,na.rm=T))+1
  ...
  days.to.today <- min(data$days.to.today,na.rm=T)
  days <- data$day[order(data$day)]
  
  start.date <- max(days,na.rm=T)-floor(max(days)/days.break)*days.break+1
  while(sum(days<start.date+days.break-1)==0){
    start.date <- start.date+days.break
  }
  
  
  opts <- expand.grid(opts.R0,npop,opts.E_0,opts.I_0)
  names(opts) <- c("R0","npop","E_0","I_0")
  ...
  results$cases.estimated.m1 <- results$cases.estimated
  results$cases.reported.m1 <- results$cases.reported
  
  r <- results[results$type=="historic",]
  r <- r[r$time==max(r$time),]
  r <- as.data.frame(r)
  r <- r[,c("time","index","cases.estimated.m1","cases.reported.m1")]
  r$time <- r$time+1
  names(r)[3] <- "cases.estimated.m1.historic"
  names(r)[4] <- "cases.reported.m1.historic"
  
  
  #results$cases.estimated.m1[2:nrow(results)] <- results$cases.estimated.m1[2:nrow(results)]-results$cases.estimated.m1[1:(nrow(results)-1)]
  #results$cases.reported.m1[2:nrow(results)] <- results$cases.reported.m1[2:nrow(results)]-results$cases.reported.m1[1:(nrow(results)-1)]
  
  resultsx <- merge(results,r,by=c("index","time"),all=TRUE)
  resultsx <- data.table(resultsx,key=c("type","index","time"))
  resultsx$cases.estimated.m1 <- resultsx$cases.estimated
  resultsx$cases.estimated.m1[2:nrow(resultsx)] <- resultsx$cases.estimated.m1[1:(nrow(resultsx)-1)]
  resultsx$cases.estimated.m1[!is.na(resultsx$cases.estimated.m1.historic)] <- resultsx$cases.estimated.m1.historic[!is.na(resultsx$cases.estimated.m1.historic)]

  resultsx$cases.reported.m1 <- resultsx$cases.reported
  resultsx$cases.reported.m1[2:nrow(resultsx)] <- resultsx$cases.reported.m1[1:(nrow(resultsx)-1)]
  resultsx$cases.reported.m1[!is.na(resultsx$cases.reported.m1.historic)] <- resultsx$cases.reported.m1.historic[!is.na(resultsx$cases.reported.m1.historic)]

 resultsx$new.cases.estimated <- resultsx$cases.estimated - resultsx$cases.estimated.m1
 resultsx$new.cases.reported <- resultsx$cases.reported - resultsx$cases.reported.m1
 resultsx <- resultsx[,cases.estimated.m1:=NULL]
 resultsx <- resultsx[,cases.reported.m1:=NULL]
 resultsx <- resultsx[,cases.estimated.m1.historic:=NULL]
 resultsx <- resultsx[,cases.reported.m1.historic:=NULL]
  results <- resultsx
  ...
  collapsed <- adj.results[,
           ...
           T=sum(T*adjp),
           T2=sum(T^2*adjp),
           new.cases.estimated=sum(new.cases.estimated*adjp),
           new.cases.estimated2=sum(new.cases.estimated^2*adjp),
  		   new.cases.reported=sum(new.cases.reported*adjp),
           new.cases.reported2=sum(new.cases.reported^2*adjp)
  				 ),
                      by=list(time,type)]
  ...
  collapsed$new.cases.estimated.sd <- sqrt(collapsed$new.cases.estimated2-collapsed$new.cases.estimated^2)
  collapsed$new.cases.estimated.sd[is.nan(collapsed$new.cases.estimated.sd)] <- 0
  
  collapsed$new.cases.reported.sd <- sqrt(collapsed$new.cases.reported2-collapsed$new.cases.reported^2)
  collapsed$new.cases.reported.sd[is.nan(collapsed$new.cases.reported.sd)] <- 0
  ...
  collapsed <- collapsed[,new.cases.estimated2:=NULL]
  collapsed <- collapsed[,new.cases.reported2:=NULL]
  ...
  collapsed$new.cases.estimated.l95 <- collapsed$new.cases.estimated-1.96*collapsed$new.cases.estimated.sd
  collapsed$new.cases.estimated.u95 <- collapsed$new.cases.estimated+1.96*collapsed$new.cases.estimated.sd
  
  collapsed$new.cases.reported.l95 <- collapsed$new.cases.reported-1.96*collapsed$new.cases.reported.sd
  collapsed$new.cases.reported.u95 <- collapsed$new.cases.reported+1.96*collapsed$new.cases.reported.sd
...
for(i in names(data.models)){
	...
	if(i!="sierra"){
		results.models[[i]] <- modelSEIR(data.models[[i]],npop=npop,opts.R0[1],opts.E_0[1],opts.I_0[1])
 	} else results.models[[i]] <- modelSEIR(data.models[[i]],npop=npop,opts.R0,opts.E_0,opts.I_0)
}
	...
	list(
	  ...
	  cases.reported.sd=sum(cases.reported.sd^2),
		cases.observed=sum(cases.observed),
         ...
         T=sum(T),
         T.sd=sum(T.sd^2),
         new.cases.estimated=sum(new.cases.estimated),
         new.cases.estimated.sd=sum(new.cases.estimated.sd^2),
         new.cases.reported=sum(new.cases.reported),
         new.cases.reported.sd=sum(new.cases.reported.sd^2)
    ),
...
aggregated$new.cases.estimated.sd <- sqrt(aggregated$new.cases.estimated.sd)
aggregated$new.cases.estimated.l95 <- aggregated$new.cases.estimated-1.96*aggregated$new.cases.estimated.sd
aggregated$new.cases.estimated.u95 <- aggregated$new.cases.estimated+1.96*aggregated$new.cases.estimated.sd

aggregated$new.cases.reported.sd <- sqrt(aggregated$new.cases.reported.sd)
aggregated$new.cases.reported.l95 <- aggregated$new.cases.reported-1.96*aggregated$new.cases.reported.sd
aggregated$new.cases.reported.u95 <- aggregated$new.cases.reported+1.96*aggregated$new.cases.reported.sd
...
plotting.models <- labeled.data
ybreaks <- c(100,1000,2000,5000,10000,15000,20000,30000,40000)
ylabs <- paste(ybreaks/1000,"K",sep="")

for(j in names(plotting.models)){
  plotting.models[[j]] <- plotting.models[[j]][plotting.models[[j]]$label=="Historical",]
  xbreaks <- seq(-28*7,0,28)
  
  q <- ggplot(plotting.models[[j]],aes(x=days.to.today))
  for(i in ybreaks) q <- q + geom_hline(yintercept=(i),lty=3,lwd=2)
  for(i in xbreaks) q <- q + geom_vline(xintercept=i,lty=3,lwd=2)
  q <- q + geom_point(aes(y=(cases.observed)),size=20)
  
  q <- q + geom_ribbon(aes(ymin=(cases.reported.l95),ymax=(cases.reported.u95),fill="red"),
                       alpha=0.4)
  q <- q + geom_line(aes(y=(cases.reported),col="red"),lwd=3)
  
  q <- q + geom_ribbon(aes(ymin=(cases.estimated.l95),ymax=(cases.estimated.u95),fill="blue"),
                       alpha=0.4)
  q <- q + geom_line(aes(y=(cases.estimated),col="blue"),lwd=3)
  
  q <- q + scale_colour_manual("",values=c("blue"="blue","red"="red"),labels=c("Corrected","Reported"))
  q <- q + scale_fill_manual("",values=c("blue"="blue","red"="red"),labels=c("Corrected","Reported"))
...
temp <- plotting.models[["sierra"]]
temp[1,]
temp <- temp[,c("day","cases.reported","cases.observed"),with=FALSE]
temp <- na.omit(temp)
temp$newrep <- NA
temp$newrep[nrow(temp):2] <- temp$cases.reported[nrow(temp):2]-temp$cases.reported[(nrow(temp)-1):1]
temp$newobs <- NA
temp$newobs[nrow(temp):2] <- temp$cases.observed[nrow(temp):2]-temp$cases.observed[(nrow(temp)-1):1]
temp$dif <- temp$newobs-temp$newrep
summary(lm(temp$dif~temp$day))

plotting.models <- labeled.data
for(j in names(plotting.models)){
	plotting.models[[j]]$place <- j
}
plotting.models <- rbindlist(plotting.models,fill=TRUE)
ybreaks <- c(100,500,1000,2000,5000,10000,20000,40000)
ylabs <- paste(ybreaks/1000,"K",sep="")

plotting.models <- plotting.models[plotting.models$label=="Historical",]
xbreaks <- seq(-28*5,0,28)

plotting.models$place <- factor(plotting.models$place)
levels(plotting.models$place) <- c("\nGuinea",
	"\nLiberia",
	"\nSierra Leone",
	"\nTotal")
plotting.models$place <- factor(plotting.models$place,levels=c(
	"\nTotal",
	"\nSierra Leone",
	"\nLiberia",
	"\nGuinea"))
  
q <- ggplot(plotting.models,aes(x=days.to.today))
  for(i in ybreaks) q <- q + geom_hline(yintercept=log2(i),lty=3,lwd=2)
  for(i in xbreaks) q <- q + geom_vline(xintercept=i,lty=3,lwd=2)
  ...
  q <- q + geom_ribbon(aes(ymin=log2(cases.reported.l95),ymax=log2(cases.reported.u95),fill="blue"),
                       alpha=0.4)
  q <- q + geom_line(aes(y=log2(cases.reported),col="blue"),lwd=5)
  
  q <- q + geom_ribbon(aes(ymin=log2(cases.estimated.l95),ymax=log2(cases.estimated.u95),fill="red"),
                       alpha=0.4)
  q <- q + geom_line(aes(y=log2(cases.estimated),col="red"),lwd=5)
  ...
  q <- format_plot(q,6,10)
  q <- q + scale_y_continuous("Cumulative cases\n",breaks=log2(ybreaks),labels=ylabs,
  	lim=c(log2(min(plotting.models$cases.observed,na.rm=T)-5),log2(40100)))
    png(paste(today.character,"/",today.character,"_log_validation_ALL.png",sep=""),width=3508,height=2480)
  print(q)
  dev.off()
...
plotting.models <- labeled.data
for(j in names(plotting.models)){
  plotting.models[[j]]$new.cases.estimated.l95[plotting.models[[j]]$new.cases.estimated.l95<5] <- 5
  plotting.models[[j]]$new.cases.estimated.l95[plotting.models[[j]]$new.cases.estimated.l95>plotting.models[[j]]$new.cases.estimated] <- plotting.models[[j]]$new.cases.estimated[plotting.models[[j]]$new.cases.estimated.l95>plotting.models[[j]]$new.cases.estimated]
  q <- ggplot(plotting.models[[j]],aes(x=days.to.today))
  for(i in ybreaks) q <- q + geom_hline(yintercept=log2(i),lty=3,lwd=2)
  for(i in xbreaks) q <- q + geom_vline(xintercept=i,lty=3,lwd=2)
#  q <- q + geom_line(aes(y=log2(new.cases.reported),col="blue"),lwd=3)
	q <- q + geom_ribbon(data=plotting.models[[j]][plotting.models[[j]]$label=="Projection" | plotting.models[[j]]$label=="Historical",],mapping=aes(ymin=log2(new.cases.estimated.l95),ymax=log2(new.cases.estimated.u95),fill=label),
                       alpha=0.4)

...
fitSEIR <- function(simulation.start,simulation.end,opts,data,mod,reported.to.date){
  predictions <- list()
  opts$dif <- 0
  opts$obs.end <- 0
...
trySEIR <- function(simulation.start,simulation.end,opts,data,mod,reported.to.date){
  retval <- fitSEIR(simulation.start,simulation.end,opts,data,mod,reported.to.date)
  #return(retval)
...
nextstepSEIR <- function(simulation.start,simulation.end,last.step,R0,data,mod){ 
  save.opts <- list()
  for(i in 1:length(last.step)){
    ...
    new.ops <- trySEIR(simulation.start,simulation.end,opts,data,mod,opts.of.interest$reported.end)
...
step1 <- trySEIR(start.date,start.date+days.break-1,opts,data,mod,0)
...
while(first.day<max(days)){
  ...
  print(max(days))
  print(length(step1))
  step1 <- nextstepSEIR(first.day-1,first.day+days.break-1,step1,opts.R0,data,mod)
  step1 <- slim.options(step1)

...
SEIR <- function(simulation.start,simulation.end,R0,time.incubation,time.infectedL,time.infectedD,cfr,
  ...
  vparameters = c(
    gamma.leaving.I=gamma.leaving.I,
    gamma.I.T=gamma.I.T,
    gamma.I.R=gamma.I.R,
    gamma.I.D=gamma.I.D,
    gamma.leaving.T=gamma.leaving.T,
    gamma.T.TR=gamma.T.TR,
    gamma.T.TD=gamma.T.TD,
    beta=beta,mu=mu,a=a,reported=reported,cfr=cfr)
...
for(j in names(new.day)){
  ...
  titles$cases.reported <- "Cases (reported/treated)"
  titles$new.cases.reported.treated <- "New cases/day (reported/treated)"
  titles$cases.estimated <- "Cases (total)"
  titles$new.cases.estimated <- "New cases/day (total)"
  ...
  write.table(to.print,file=paste(today.character,"/",today.character,"_written_estimates_",j,".csv",sep=""),row.names=FALSE,col.names=FALSE,sep=";",qmethod="double")
}
